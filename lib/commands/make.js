...
function bundleFlavor(options, callback) {
  ...
  if (!name) name = getBundleFileName(options)
  rimraf.sync(temp)
...
function getBundleFileName(options) {
  var bundleFileName = properties[options.packageConfig + ".packageFileName"] || properties["packageFileName"] || options.flavor
  let version = utils.getVersionStringFromManifest(options)
  let gitBranch = utils.getCurrentGitBranch()

  bundleFileName = bundleFileName.replace('%flavor%', options.flavor)
  bundleFileName = bundleFileName.replace('%version%', version)
  bundleFileName = bundleFileName.replace('%branch%', gitBranch)
